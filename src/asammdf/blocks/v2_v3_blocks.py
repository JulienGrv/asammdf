    ...
    def __init__(self, **kwargs: Unpack[ChannelConversionKwargs]) -> None:
        ...
        if "raw_bytes" in kwargs or "stream" in kwargs:
            ...
            (
                ...
                self.ref_param_nr,
            ) = CONVERSION_COMMON_SHORT_uf(block, 0)
    ...
    * ``path`` - str : extension path
    ...
    def __init__(self, **kwargs: Unpack[_ChannelExtensionKwargs]) -> None:
        ...
        if "stream" in kwargs:
            stream = kwargs["stream"]
            try:
                (self.id, self.block_len, self.type) = SOURCE_COMMON_u(kwargs["raw_bytes"])
                ...
                if utils.stream_is_mmap(stream, kwargs.get("mapped", False)):
                    ...
                    (self.id, self.block_len, self.type) = SOURCE_COMMON_u(block)
